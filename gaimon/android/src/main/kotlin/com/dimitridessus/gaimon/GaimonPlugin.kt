...
class GaimonPlugin: FlutterPlugin, MethodCallHandler {
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result) {
    when (call.method) {
      ...
      "hapticsFromData" -> {
        val callArgs: HashMap<String, Any> = call.arguments as HashMap<String, Any>

        val timings = (callArgs["timings"] as ArrayList<Long>).toLongArray()
        val scales = (callArgs["scales"] as ArrayList<Int>).toIntArray()
        val repeat = if (callArgs["repeat"] as Boolean) 1 else -1

        if (vibrator.hasAmplitudeControl()) {
          val effect = VibrationEffect.createWaveform(timings, scales, repeat)
          vibrate(effect)
        }
      }
      "cancel" -> {
